...
import profiles from './Table.json' // eslint-disable-line import/extensions
...
export default class AdvancedTable extends React.Component {
  ...
  sort = profiles => {
    ...
    if (orderedColumn === 2) propKey = 'username'
    // The third column is fixed to the `ltv` property.
    if (orderedColumn === 3) propKey = 'user role'

    if (orderedColumn === 3) propKey = 'last online'
  ...
  renderRow = ({ profile }) => {
    return (
      <Table.Row key={profile.id}>
        ...
        <Table.TextCell>{profile.userrole}</Table.TextCell>
        <Table.TextCell>{profile.lastonline}</Table.TextCell>
    ...
    return (
      <Table border>
        <Table.Head>
          ...
          <Table.HeaderCell width={10} flex="none" />
        </Table.Head>
